...
export const Loading: React.FC<LoadingProps> = ({
  ...
  if (fullscreen) {
    return (
      <div className="fixed inset-0 bg-card/90 backdrop-blur-sm flex items-center justify-center z-50">
        {content}
      </div>
    );
  }
...
export const Skeleton: React.FC<{ className?: string }> = ({ className }) => {
  return (
    <div
      className={cn(
        'animate-shimmer bg-gradient-to-r from-muted via-accent to-muted',
        'bg-[length:200%_100%]',
        className
      )}
    />
